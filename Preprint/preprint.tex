...
Large language models have recently been extended to speech generation, with text-to-speech systems increasingly adopting autoregressive architectures originally designed for text. However, most existing TTS systems built on language models focus on a single capability in isolation: either multilingual synthesis, or voice cloning, or expressive control, each requiring task-specific architectures, dedicated training pipelines, and separate model checkpoints. This fragmentation limits our understanding of how these capabilities interact, whether they benefit from shared representations, and how they scale jointly under a unified training framework.
 
A parallel limitation concerns the representation of speech itself. The dominant paradigm in LLM-based speech generation relies on multi-codebook neural codec representations, where each audio frame is encoded into multiple residual quantization levels. While this captures fine-grained acoustic detail, it introduces architectural complexity: the model must either predict multiple codebook streams in parallel, employ cascaded decoders, or use masking strategies to resolve inter-codebook dependencies. These design choices couple the model tightly to the codec architecture and complicate scaling analysis, since varying one component (e.g., the number of codebook levels) affects both model capacity requirements and generation quality in ways that are difficult to disentangle.
 
In contrast, single-codebook discrete tokenizers such as NeuCodec (\citet{julian2025finitescalarquantizationenables}) produce a flat sequence of token IDs from a single vocabulary, analogous to how a text tokenizer converts sentences into subword IDs. This representation allows speech to be incorporated into the language model's vocabulary as standard token IDs, requiring no architectural modification, no multi-stream decoding, and no residual quantization hierarchy. The model simply learns to predict the next speech token given all preceding tokens, exactly as it does for text. This architectural simplicity makes it possible to study how speech generation scales with model size, data volume, and task diversity without confounding variables introduced by codec-specific design choices.
 
Building on this observation, we investigate whether a single autoregressive LLM can jointly acquire multilingual speech synthesis, voice cloning, and expressive style control from discrete audio tokens alone. We continue pretraining Qwen3 (\citet{yang2025qwen3technicalreport}) 0.6B and 1.7B models across three staged training phases, each targeting a distinct capability. The first phase trains on a base multilingual TTS corpus covering more than 150 languages, where each sample pairs a speaker-conditioned text prompt with its corresponding NeuCodec token sequence. The second phase introduces voice conversion training using paired reference-target utterances, where speaker pairs are constructed by filtering for high cosine similarity between TitaNet (\citet{koluguri2022titanet}) speaker embeddings. The third phase performs expressive TTS post-training on a dataset where acoustic features including pitch, emotion, speaking rate, fluency, and noise characteristics are extracted from audio, binned into discrete categories, and converted into natural-language style descriptions by Qwen2.5-72B-Instruct (\citet{qwen2025qwen25technicalreport}). These descriptions serve as conditioning signals that teach the model to control speech style through natural language.
 
All audio across all training stages is tokenized by NeuCodec at 50 tokens per second using a single codebook of 65,536 entries. The resulting token sequences are concatenated with text tokens and packed into fixed 10,240-token chunks using a dynamic multipacking strategy that eliminates padding waste and maintains near-complete GPU utilization. Training is performed with FlashAttention-3 (\citet{shah2024flashattention3fastaccurateattention}) and the Liger fused cross-entropy kernel (\citet{hsu2025ligerkernelefficienttriton}) to handle the large vocabulary efficiently.
 
Beyond the core training pipeline, we report an optimizer ablation finding with broader implications for speech token learning. We evaluate a hybrid Muon+AdamW (\citet{muon}) strategy that applies the Muon optimizer to two-dimensional hidden layer weights while retaining AdamW for embedding tables and the output projection head. Under aggressive learning rate schedules, this hybrid consistently outperforms AdamW alone, suggesting that the optimization landscape for discrete speech token prediction benefits from separating the treatment of structured weight matrices from unstructured embedding parameters.
 
Because the model operates entirely over discrete token IDs, with no continuous encoder features or projection layers, it is fully compatible with standard LLM inference infrastructure. Prefix caching works natively, enabling efficient streaming generation. The same model checkpoint serves all three tasks: given a speaker name and text, it synthesizes speech; given a reference audio prompt, it clones the speaker's voice; and given a natural-language style description, it generates expressive speech with controlled prosody. This unification under a single discrete-token framework, spanning 150+ languages and three generation capabilities, has not been previously demonstrated.
 
We evaluate on a 76-language benchmark spanning CER, MOS, and speaker similarity metrics. The 1.7B variant achieves a CER of 0.2362 and MOS of 3.2330, outperforming Dia TTS, Orpheus, and Fish Audio S2 Pro on the full multilingual evaluation. For voice cloning, the same model reaches a speaker similarity of 0.5036 with a CER of 0.2656 across 76 languages.
