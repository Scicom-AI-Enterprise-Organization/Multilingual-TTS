...
MODEL_NAME = os.environ.get('MODEL_NAME', 'Scicom-intl/Multilingual-Expressive-TTS-1.7B')
...
def loop(indices_device_pair):
    ...
    for r in tqdm(rows):
        ...
        try:
            prompt = f"<|im_start|>{speaker}: {r['target_text']}<|speech_start|>"
...
def main(output, speaker, replication, retry):
